    ...
    arcgisRest.queryFeatures({
        ...
        $("#fdiGrid").kendoGrid({
            ...
            columns: [{
                ...
                width: "15%",
                // template: "#=kendo.toString(Math.round(Employees/10)*10,'n0')#",
                template:"#if(Employees<10){# #=Employees # #}else{# #=kendo.toString(Math.round(Employees/10)*10,'n0')# #}#", //TESTING ROUNDING
                headerAttributes: {
                    style: "font-weight:bold;"
                }
            }]
        ...
        $("#fdiChart").kendoChart({
            ...
            tooltip: {
                visible: true,
                // template: "<strong>#=category #</strong><br>#=kendo.toString(Math.round(value/10)*10,'n0')# Jobs"
                template:"#if(value<10){# <strong>#=category #</strong><br>#=value# Jobs #}else{# <strong>#=category #</strong><br>#=kendo.toString(Math.round(value/10)*10,'n0')# Jobs #}#" //ROUNDING
            }
        });
    });
...
$(".fdireset").click(function () { //move this outside?
    ...
    arcgisRest.queryFeatures({
        ...
        $("#clusterchartfdi").kendoChart({
            ...
            seriesDefaults: {
                type: "bar",
                gap: 0.5,
                labels: {visible:false, template:"#=kendo.toString(Math.round(value/10)*10,'n0')#",font:"10px Open Sans, sans-serif"}
            },
            chartArea:{
                height:500,
            },
    ...
    arcgisRest.queryFeatures({
        ...
        //TESTING dynamic grid height
        if (totalBus==1){
            top20height=3.5+32+32*totalBus; //how to make this auto?
            $("#top20gridtitle").html("");
            // $(".top20sharesection").hide(); //hide section if less than 20
        }
        else if (totalBus<20){
            top20height=32+32*totalBus; //how to make this auto?
            $("#top20gridtitle").html(""); //hide section if less than 20
            // $(".top20sharesection").hide();
        }
        else{
            top20height=400;
            $("#top20gridtitle").html("Top 20");
            // $(".top20sharesection").show(); //show section if 20+
        }
        //END TESTING
    ...
    function calculateKI(){
        ...
        $("#keyindchart").kendoChart({
            ...
            seriesDefaults: {
                ...
                labels: {
                    visible:true, 
                    // template:"#=kendo.toString(Math.round(value/10)*10,'n0')#",
                    template:"#if(value<10){# #=value # #}else{# #=kendo.toString(Math.round(value/10)*10,'n0')# #}#", //ROUNDING
    ...
    arcgisRest.queryFeatures({
        ...
        $("#clusterchart").kendoChart({
            ...
            seriesDefaults: {
                ...
                labels: {
                    visible:false, 
                    // template:"#=kendo.toString(Math.round(value/10)*10,'n0')#",font:"bold 11px Open Sans, sans-serif"
                    template:"#if(value<10){# #=value # #}else{# #=kendo.toString(Math.round(value/10)*10,'n0')# #}#", //ROUNDING
                }
            },
            chartArea:{
